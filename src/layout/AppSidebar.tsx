...
import { useCallback, useEffect, useRef, useState } from "react";
import { Link, useLocation, useNavigate } from "react-router";
import { LogOut, Warehouse, Smartphone, ClockAlert, PackageCheck, LayoutDashboard, Truck, File } from "lucide-react";
                  ...
                  <div className="mt-2 space-y-2 overflow-hidden transition-all duration-300">
                    {nav.subItems.map((subItem) => (
                      <Link
                        ...
                        title={subItem.name}
                      >
                        <span className={`${isActive(subItem.path) ? "text-brand-500" : "text-gray-600"}`}>
                          {subItem.icon}
                        </span>
                      </Link>
                    ))}
