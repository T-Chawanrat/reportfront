...
export default function VtgToday() {
  const [vtgToday, setVtgToday] = useState<VtgToday[]>([]);
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const res = await AxiosInstance.get("/today", { params });

      setVtgToday(res.data.data || []);
      ...
      <div className="overflow-x-auto w-full">
        ...
        <div className="overflow-x-auto w-full">
          <table className="w-full table-fixed border border-gray-300 rounded overflow-hidden">
            <ResizableColumns headers={headers} pageKey="vtgToday" />
            <tbody>
              {vtgToday.map((t, i) => (
